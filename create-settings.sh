...
if [ -z "$NEXUS_REGISTRY_URL" ] || [ -z "$NEXUS_USERNAME" ] || [ -z "$NEXUS_PASSWORD" ]; then
    >&2 echo "Error - NEXUS_REGISTRY_URL, NEXUS_USERNAME, and NEXUS_PASSWORD environment variables are required"
    exit 1
fi

echo "registry=$NEXUS_REGISTRY_URL" > $DIR/.npmrc
echo "_auth=$(echo -n "$NEXUS_USERNAME:$NEXUS_PASSWORD" | base64)" >> $DIR/.npmrc
